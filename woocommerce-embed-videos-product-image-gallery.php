...
/**
  ...
  **/
add_filter( 'attachment_fields_to_edit', 'woo_embed_video', $post->ID, 2);
function woo_embed_video( $form_fields, $post ) { ?>
    <?php 
    $field_value = get_post_meta( $post->ID, 'videolink_id', true );
    $video_site = get_post_meta( $post->ID, 'video_site', true );
    ...
    $form_fields['video_site'] = array(
        'input' => 'html',
        'value' => $video_site,
        'html' => "<input type='radio' name='attachments[{$post->ID}][video_site]' value='youtube' $youtube $checked> Youtube
                   <input type='radio' name='attachments[{$post->ID}][video_site]' value='vimeo' $vimeo> Vimeo",                   
